...
class PDFExtractor:
    ...
    def _extract_pages_data(self):
        for page_num, page in enumerate(self.pdf.pages, 0):
            chars = page.chars
            if chars:
    ...
    def _group_adjacent_lines(self, prominent_lines: List[Dict]) -> List[List[Dict]]:
        ...
        for i in range(1, len(prominent_lines)):
            current_line = prominent_lines[i]
            prev_line = prominent_lines[i-1]
            
            if self._lines_can_be_grouped(prev_line, current_line):
                current_group.append(current_line)
            else:
                # End current group, start new one
                if len(current_group) >= 1:  # Keep single lines too
    ...
    def _lines_can_be_grouped(self, line1: Dict, line2: Dict) -> bool:
        """Check if two lines can be grouped as part of same multiline title"""
        
        # Font size similarity (within tolerance)
        font_diff = abs(line1['avg_font_size'] - line2['avg_font_size'])
        if font_diff > self.FONT_SIZE_TOLERANCE:
            return False
        
        # Vertical spacing check
        y1 = line1.get('y_position', 0)
        y2 = line2.get('y_position', 0)
        height1 = line1.get('height', 14)
        
        vertical_gap = abs(y2 - (y1 + height1))
        max_allowed_gap = height1 * self.MAX_LINE_SPACING
        
        if vertical_gap > max_allowed_gap:
            return False
        
        # Horizontal alignment similarity
        pos1 = line1.get('position', 'left')
        pos2 = line2.get('position', 'left')
        
        if pos1 != pos2:
            return False
        
        # Bold/formatting consistency (optional - be lenient)
        # Allow some variation in bold formatting for multiline titles
        
        return True
    
    def _is_valid_title_group(self, group: List[Dict], all_lines_data: List[Dict]) -> bool:
        """Validate if a group of lines forms a valid multiline title"""
        
        if not group:
            return False
        
        # Check group size limits
        if len(group) > self.MAX_TITLE_LINES:
            return False
        
        # Find group position in all_lines_data for whitespace analysis
        first_line_index = None
        last_line_index = None
        
        for i, line in enumerate(all_lines_data):
            if line['text'].strip() == group[0]['text'].strip():
                first_line_index = i
                break
        
        for i, line in enumerate(all_lines_data):
            if line['text'].strip() == group[-1]['text'].strip():
                last_line_index = i
                break
        
        if first_line_index is None or last_line_index is None:
            return True  # Can't verify spacing, assume valid
        
        # Check whitespace above and below group
        whitespace_above = self._measure_whitespace_above_line(all_lines_data, first_line_index)
        whitespace_below = self._measure_whitespace_below_line(all_lines_data, last_line_index)
        
        # Require significant whitespace around title group
        if (whitespace_above < self.MIN_WHITESPACE_ABOVE and first_line_index > 0):
            return False
        
        if (whitespace_below < self.MIN_WHITESPACE_BELOW and last_line_index < len(all_lines_data) - 1):
            return False
        
        # Check for obvious non-title content
        combined_text = ' '.join([line['text'].strip() for line in group])
        
        # Reject if it looks like form fields or metadata
        if (re.search(r'\d+\.\s+[A-Z][a-z\s]*:?\s*$', combined_text) or
            'signature' in combined_text.lower() or
            'date' in combined_text.lower() and len(combined_text) < 30):
            return False
    ...
    def _extract_title_by_multiline(self, lines_data: List[Dict]) -> Optional[Dict]:
        ...
        # Get spatial bounds of the group
        ...
        return {
            ...
            'score': best_score,
            'group_lines': [line['text'].strip() for line in best_group]  # For debugging
        }
    ...
    def extract_headings(self) -> List[Dict]:
        """Legacy method - now calls title-aware heading extraction"""
        return self.extract_headings_with_title_awareness()
    
    def extract_headings_with_title_awareness(self) -> List[Dict]:
        """Extract headings while being aware of document title"""
        ...
        # Step 3: Remove title from heading candidates
        if title_info['title']:
            headings = self._exclude_title_from_headings(headings, title_info)
        
        # Step 4: Apply title-aware font hierarchy
        headings = self._apply_title_aware_hierarchy(headings, title_info, all_lines_data)
        
        # Step 5: Apply spatial enhancement
        enhanced_headings = self.hierarchy_enhancer.enhance_headings(headings, all_lines_data)
        
        # Step 5.5: Apply strict isolation filter to remove false positives
        filtered_headings = self._filter_false_positive_headers(enhanced_headings, all_lines_data)
        
        # Step 5.6: Final filter - only allow H1, H2, H3 level headers
        major_headings = self._filter_to_major_headings_only(filtered_headings)
        
        return self._classify_and_clean_headings(major_headings)
    ...
    def _has_section_title_characteristics(self, text: str) -> bool:
        ...
        title_words = ['introduction', 'overview', 'background', 'methodology', 'results', 'conclusion', 
        ...
        if len(words) >= 2:
            ...
            if capitalized >= len(words[1:]) * 0.7:  # 70% of words capitalized
                return True
    ...
    def extract_data(self) -> Dict:
        """Extract both title and headings with proper hierarchy"""
        
        # Extract title first with comprehensive method
        title_info = self._extract_document_title_comprehensive()
        
        # Extract headings with title awareness
        outline = self.extract_headings_with_title_awareness()
